...
function UserOrderHistory() {
  ...
  const { orders, loading, error, getUserOrders, cancelOrder, pagination } = useOrder();

  useEffect(() => {
    const loadOrders = async () => {
      try {
        await getUserOrders(currentPage, 10, statusFilter);
      } catch (error) {
    ...
    loadOrders();
  }, [currentPage, statusFilter, getUserOrders]);
  ...
  const toggleOrderExpansion = (orderId) => {
    setExpandedOrders(prev => {
      ...
      const id = orderId._id || orderId.id || orderId;
      if (newSet.has(id)) {
        newSet.delete(id);
      } else {
        newSet.add(id);
